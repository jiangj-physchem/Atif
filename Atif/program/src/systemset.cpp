...
namespace systemset
{
    void readSystem(short& nspecies1,PolyInfo* pInfo,Species* species,SysInfo& sysInfo,IteraInfo& iteraInfo,
                    double* rhoB,double** pairEner,double& eta0,short** mb,float** zb,string& filePath)
    {
        ...
        inMark.open(markPath.c_str(),ios_base::in);
        while(!inMark && iDirect < 10)
        {
            markPath = "../" + markPath;
            inMark.open(markPath.c_str(),ios_base::in);
            ++iDirect;
        ...
        if(iDirect < 10)
        {
            ...
            {
                getline(inMark,mark[i]);
            }
        }
        else
        {
            cerr<<"Error in systemset.cpp: fail to open the mark file"<<endl;
            exit(0);
        }
        inMark.close();
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void readSystem(PolyInfo* pInfo,MethodGeom& MethG)
    {
        ios::sync_with_stdio(false);
        cin.tie(0);
        string s;
                    ...
                    if(getline(inFile,s))
                    {
                        ...
                        do{
                                ...
                                || k==(len-1))&&(n==0))
                            {
                                ...
                                if(j==2) MethG.a = atof(temp2);
                                if(j==3) MethG.b = atof(temp2);

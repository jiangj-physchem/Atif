                   ...
                   double** ff1,double** ff2,double** rho1,double** rho2,double*** BesselZero,string* MODEL)
{
    ...
    for(int k=(ngrid_m+1); k<=(MAXR0+1); ++k)
    {
        for(short i=0; i<hspecies; ++i)
        {
            rho2[i][k] = rho2[i][ngrid-k];
        }
    }
    ...
    for(int k=LLIM; k<=ngrid_m; ++k)
    {
        VolumeFraction(k,LLI,ULI,D,rho2,eta);
        eta_max = 0;
        for(short i=0; i<hspecies; ++i)
        {
            eta_max = eta_max + eta[i];
        }
        ...
        if(eta_max > 0.74)
        {
            for(short i=0; i<hspecies; ++i)
            {
                if(eta[i] > etar[i]) rho2[i][k] = rho2[i][k]*etar[i]/eta[i];
            }
        }
    }
    //////////////////////////////////////////////////////////////////////
    ...
    if(ngridm0%2 != 0)
    {
        ...
        for(short i=0; i<hspecies; ++i)
        {
            fsigma += (rho2[i][ngridm0] + rho2[i][ngrid_m])*Z[i]*1.5;
        }
    }
    for(short i=0; i<hspecies; ++i)
    {
        fsigma += (rho2[i][0] + rho2[i][ngridm0])*Z[i];
        fsigma += rho2[i][ngridm0-1]*Z[i]*4.0;
        for(int k=1; k<(ngridm0-2); k=(k+2))
        {
            fsigma += rho2[i][k]*Z[i]*4.0;
            fsigma += rho2[i][k+1]*Z[i]*2.0;
        }
        if(ngrid%2 != 0) fsigma += (rho2[i][ngrid_m] + rho2[i][ngrid-ngrid_m-1])*Z[i]*0.75;
    }
    fsigma = fsigma*dr/3.0;
     */

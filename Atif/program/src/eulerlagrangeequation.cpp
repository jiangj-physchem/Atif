                   ...
                   double*** BesselZero,double** rho,double** rho1,string* MODEL)
{
    ...
    double  rhoBM1,rhoBM2,D_2,Dav_1,Dav_2,VS,LNRHOS; //thresh
    double* VI;
    double* temp;
    double* eta;
    double* u_im;
    ...
    if(neutsys == 1)
    {
        ...
        PoissonEquation(Z,rho,phi,0,Psi);
    ...
    for(int k=LLIM; k<=ngrid_m; ++k)
    {
        for(short j=nblocks; j<hspecies; ++j)
        {
            ...
            {
                LNRHOS = VI[j]*log(rho[hspecies][k]);
                
                rho1[j][k] = exp(mu[j]+LNRHOS-Ext[j][k]-VanDW[j][k]-Psi[k]*Z[j]-Z[j]*Z[j]*u_im[k]);
        ...
        if(rhoBM1 > 1E-10)
        {
            for(short j=0; j<nb[0]; ++j)
            {
                ...
                if(Ext[j][k]<1.0E10)
                {
                    ...
                    temp[j] = exp(mu[j]+LNRHOS-Ext[j][k]-VanDW[j][k]-Psi[k]*Z[j]-Z[j]*Z[j]*u_im[k]);
                }
            }
    ...
    if(rhoBM1 > 1E-10)
    {
        ...
        else if(MODEL[0] == "rod")//rod chain
        {
            ...
            fL1  = new double**[mp1]();
            fR1  = new double**[mp1]();
            //cout<<"test-0-0"<<" Dav_1="<<Dav_1<<" orient1="<<orient1<<" mp1="<<mp1<<endl;
        ...
        else if(MODEL[0] == "semiflexible" || MODEL[0] == "semi-flexible")//stiff chain
        {
                ...
                {//loop for Z_1
                    ...
                    {//loop for Z_2
                        ...
                        {//loop for Z_0
                            ...
                            //if(k==ngrid_m)  cout<<"delta_Z0="<<delta_Z0<<" delta_Z1="<<delta_Z1<<" f_1="<<f_1[k2]<<" "<<BesselZero[0][delta_Z0][delta_Z1]<<endl;
                        }//loop for Z_0
                        ...
                        //    for(int k2=kin1; k2<=kip1; ++k2)
                        //    {
                        //         cout<<"test5-1 "<<"k1="<<k1<<" k2="<<k2<<" f_1[k2]="<<f_1[k2]<<" f_2[k2]="<<f_2[k2]<<" ftemp1="<<ftemp1<<endl;
                        //    }
                    ...
                    //if(k==ngrid_m) cout<<"test5-1 "<<" f_1[kin1]="<<f_1[kin1]<<" f_2[kin1]="<<f_2[kin1]<<" kin1="<<kin1<<" "<<endl;
            ...
            ////////////////////////////////////////////////////////////////////////
            //for(int k=LLIM; k<=(LLIM+20); ++k)
            //{
            //    cout<<"k="<<k<<"  rho_s1[0][k]="<<rho_s1[0][k]<<"  rho_s1[10][k]="<<rho_s1[10][k]<<"  rho_s1[20][k]="<<rho_s1[20][k]<<endl;
            //}
            //for(int k=(ngrid_m-20); k<=ngrid_m; ++k)
            //{
            //    cout<<"k="<<k<<"  rho_s1[0][k]="<<rho_s1[0][k]<<"  rho_s1[10][k]="<<rho_s1[10][k]<<"  rho_s1[20][k]="<<rho_s1[20][k]<<endl;
            //}
            //exit(0);
    ...
    VolumeFraction(eta_t,D,etar,rho1);
                   ...
                   double*** BesselZero,double** rho,double** rho1,string* MODEL)
{
    ...
    for(int k=LLIM; k<=ngrid_m; ++k)
    {
        ...
        for(short j=nblocks; j<hspecies; ++j)
        {
            ...
            if((Ext[j][k]<1.0E10) && (k<=ngrid_b))
            {
                ...
                //cout<<ngrid_m<<" k="<<k<<" j="<<j<<" mu[j]="<<mu[j]<<" LNRHOS="<<LNRHOS<<" Psi[k]="<<Psi[k]<<endl;
                if(rho1[j][k] > (1000*rhoB[j])) rho1[j][k] = rhoB[j]*1000;
            }
            else if(k > ngrid_b)
            {
    ...
    if(neutsys == 1)
    {
       RenormDensity_Newton(sigma,deltaPhi,err,rhoBM1,rhoBM2,D,MB1,MB2,Z,ff1,ff2,rho1,BesselZero,MODEL);
    }
                   ...
                   double* rhoB,double*** BesselZero,string* MODEL)
{
    ...
    //cout<<"test-0-9-1"<<endl;
    
    //Square-well potential
    InhomVanDelWaal(LLI,ULI,D,rho,pairEner,VanDW);
    
    //cout<<"test-0-9-2"<<endl;
    ...
    for(short j=nblocks; j<hspecies; ++j)
    {
        LNRHOS = VI[j]*log(rho[hspecies][ngrid_m]);
        
        rhoB_S[j] = exp(mu[j]+LNRHOS-VanDW[j][ngrid_m]);
    }
    //loop-1
    for(int k=LLIM; k<=ngrid_m; ++k)
    {
        if(rhoBM1 > 1E-10)
        {
            ...
            for(short j=0; j<nb[0]; ++j)
            {
                ...
                if(k >= LLI[j])
                {
                    ...
                    temp[j]  = exp(mu[j]+LNRHOS-VanDW[j][k]);
                }

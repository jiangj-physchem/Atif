...
int main()
{
    ...
    double    alpha,time_backup,eta,R_e1,R_e2; //,variable
    //double    P,P1,P2;
    double    f,dcharge,deltaPhi,phL_L,sigma_t,sigma_t0;
    double    coe0,coe1,coe2,coe3,coe4,dr0;
    ...
    readSystem(nspecies1,pInfo,species,sysInfo,iteraInfo,rhoB1,pairEner1,eta_t,mb,zb,filePath);
    ...
    //coe1 = nm*nm/(lunit*lunit);// surface charged density from e0/lunit^2 to e0/nm^2
    ...
    iloop  = 0;
    //num_max= 0;
    FileOpen(iloop,cstep,vari_press,converge_rec,inum_sigma,contact_theorem,fileIterative,fileILoop,fileTemp,filePath);
    ...
    if(MethG[0] == "scft") EulerLagrange(LLI,ULI,D,Z,pairEner,mu,rhoB,BesselZero,MODEL);
    if(MethG[0] == "dft") EulerLagrangeDFT(gama,LLI,ULI,D,TB,Z,pairEner,mu,rhoB,ATT,BesselZero,Psi_IJ,MODEL);
    ...
    if(!inFile)
    {
        ...
        //FileOutPut(sigma_t0,dcharge,cstep,iCode,0,Z,rho,density_temp,fileTemp);
    }
    else
    {
    ...
    for(int i_num=0; i_num<cstep; ++i_num)//loop for changing the surface charge density
    //for(int i_num=iloop; i_num<=cstep; ++i_num)//loop for changing the surface charge density
    {
        ...
        do
        {
            ...
            {
                FileOutPut(sigma_t0,dcharge,cstep,iCode,iter,i_num,Z,rho,iloop_temp,fileILoop,density_temp,fileTemp);
                ++iback;
            }
        ...
        contact_theorem<<"P_bulk= "<<P_bulk<<" Contact="<<(rho[0][LLI[0]]+rho[1][LLI[1]] - QQ)<<endl;
        ...
        if(err > errTol0)
        {
            ...
            inum_sigma<<i_num<<" "<<(coe1*sigma)<<" "<<(coe1*sigma_t)<<endl;
            
            FileOutPut(sigma_t0,dcharge,cstep,iCode,0,i_num+1,Z,rho,iloop_temp,fileILoop,density_temp,fileTemp);
        }
        else
        {
            iter = 0;
        ...
        //if((remove(fileTemp.c_str()) != 0) && (iCode == 1))
        //{
        //    iCode = 0;
    ...
    FileClose(vari_press,converge_rec,inum_sigma,contact_theorem);
